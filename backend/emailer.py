...
def branded(title: str, inner: str) -> str:
    ...
    return (
        ...
        '<p style="font-family:Arial,sans-serif;font-size:10px;letter-spacing:3px;color:#8C6D62;text-transform:uppercase;margin:0 0 24px">Atelier &amp; Haute Pâtisserie</p>'
...
async def send_reminder_email(client: dict, order: dict, rtype: str, settings: dict):
    ...
    return await send_email(to=client["email"], subject=subject, html=branded(subject, inner))
...
async def send_receipt_email(client: dict, order: dict, payment: dict, receipt: dict, settings: dict):
    ...
    return await send_email(to=client["email"], subject=subject, html=branded("Payment Receipt", inner))
...
async def send_order_confirmation_email(client: dict, order: dict, settings: dict):
    ...
    return await send_email(to=client["email"], subject=subject, html=branded("Order Confirmation", inner))
