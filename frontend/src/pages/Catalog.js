...
export default function Catalog({ tab }) {
  ...
  return (
    <div data-testid="catalog-page">
      ...
      <div className="flex gap-2 mb-6 border-b border-[#EDE5DE] overflow-x-auto" data-testid="catalog-tabs">
        {TABS.map((t) => (
          <button key={t.id} onClick={() => setActive(t.id)} data-testid={`catalog-tab-${t.id}`}
            className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors whitespace-nowrap ${active === t.id ? "border-[#2B1B17] text-[#2B1B17]" : "border-transparent text-[#78665E] hover:text-[#2B1B17]"}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {!items ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3" data-testid="catalog-grid">
          {items.map((item) => (
            <div key={item.id} className={`pq-card p-3 sm:p-5 flex flex-col ${!item.active ? "opacity-55" : ""}`} data-testid={`catalog-item-${item.id}`}>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="font-serif text-base sm:text-lg text-[#2B1B17] break-words">{item.name}</p>
                  {item.servings && <p className="text-xs text-[#78665E]">{item.servings}</p>}
                  {item.description && <p className="text-xs text-[#78665E] mt-1 break-words">{item.description}</p>}
                  {item.notes && <p className="text-[11px] text-[#B9ABA2] mt-1 italic break-words">{item.notes}</p>}
                </div>
                <span className={`text-[10px] font-bold uppercase tracking-wider shrink-0 ${item.active ? "text-[#2E5A2A]" : "text-[#9E2A2B]"}`}>
