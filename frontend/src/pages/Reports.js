...
export default function Reports() {
  ...
  const ChartCard = ({ title, children, testid }) => (
    <div className="pq-card p-4 sm:p-6" data-testid={testid}>
      <p className="pq-eyebrow mb-4">{title}</p>
      <div className="h-56 sm:h-64">{children}</div>
    </div>
  );

  const ExportLink = ({ href, label, testid, shortLabel }) => (
    <a
      href={href}
      className="pq-btn-outline shrink-0 !py-2 !px-3 text-xs whitespace-nowrap"
      data-testid={testid}
    >
      <Download size={14} />
  ...
  return (
    <div data-testid="reports-page">
      ...
      {/* ============================================================
          Export toolbar — sits below the header, wraps on mobile
      ============================================================ */}
      <div className="flex flex-wrap items-center gap-2 mb-6 sm:mb-8">
        <ExportLink
          href={`${process.env.REACT_APP_BACKEND_URL}/api/export/clients`}
          label="Clients CSV"
          shortLabel="Clients"
          testid="export-clients-btn"
        />
        <ExportLink
          href={`${process.env.REACT_APP_BACKEND_URL}/api/export/orders`}
          label="Orders CSV"
          shortLabel="Orders"
          testid="export-orders-btn"
        />
        <ExportLink
          href={`${process.env.REACT_APP_BACKEND_URL}/api/export/payments`}
          label="Payments CSV"
          shortLabel="Payments"
      ...
      {/* ============================================================
          Stat cards — keeps 2-column grid on mobile
      ============================================================ */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6">
        <ChartCard title="Monthly Revenue" testid="chart-monthly-revenue">
          <ResponsiveContainer>
            <BarChart data={r.monthly_revenue}>
              ...
              <YAxis tick={{ fontSize: 11, fill: "#78665E" }} width={40} />
        ...
        <ChartCard title="Orders Per Month" testid="chart-monthly-orders">
          <ResponsiveContainer>
            <LineChart data={r.monthly_orders}>
              <CartesianGrid strokeDasharray="3 3" stroke="#F2EAE1" />
              <XAxis dataKey="name" tick={{ fontSize: 11, fill: "#78665E" }} />
              <YAxis tick={{ fontSize: 11, fill: "#78665E" }} allowDecimals={false} width={40} />
              <Tooltip />
        ...
        <ChartCard title="Most Requested Flavors" testid="chart-flavors">
          <ResponsiveContainer>
            <PieChart>
              <Pie data={r.flavor_counts} dataKey="value" nameKey="name" innerRadius={50} outerRadius={85} paddingAngle={3}>
        ...
        <ChartCard title="Revenue by Cake Type" testid="chart-revenue-by-type">
          <ResponsiveContainer>
            <BarChart data={r.revenue_by_type} layout="vertical">
              ...
              <YAxis type="category" dataKey="name" width={75} tick={{ fontSize: 11, fill: "#78665E" }} />
              <Tooltip formatter={(v) => money(v)} />
        ...
        <ChartCard title="Order Status" testid="chart-status">
          <ResponsiveContainer>
            <PieChart>
              <Pie data={r.status_counts} dataKey="value" nameKey="name" outerRadius={85} paddingAngle={3}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <div className="pq-card p-4 sm:p-6" data-testid="report-clients">
          <p className="pq-eyebrow mb-4">Client Reports</p>
          <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-5">
            <div>
              <p className="font-serif text-xl sm:text-2xl text-[#2B1B17]">{r.clients.total}</p>
              <p className="text-[11px] sm:text-xs text-[#78665E] leading-tight">Total clients</p>
            </div>
            <div>
              <p className="font-serif text-xl sm:text-2xl text-[#2B1B17]">{r.clients.new_this_month}</p>
              <p className="text-[11px] sm:text-xs text-[#78665E] leading-tight">New this month</p>
            </div>
            <div>
              <p className="font-serif text-xl sm:text-2xl text-[#2B1B17]">{r.clients.returning}</p>
              <p className="text-[11px] sm:text-xs text-[#78665E] leading-tight">Returning</p>
            </div>
          </div>
          <p className="pq-eyebrow mb-2">Most frequent clients</p>
          {r.clients.top.length === 0 ? (
            <p className="text-sm text-[#B9ABA2]">No orders yet.</p>
          ) : (
            r.clients.top.map((c) => (
              <div key={c.name} className="flex justify-between gap-3 text-sm py-1.5 border-b border-[#F2EAE1] last:border-0">
                <span className="text-[#4A3B32] truncate">{c.name}</span>
                <span className="text-[#78665E] shrink-0">{c.orders} orders</span>
              </div>
            ))
          )}
        </div>

        <div className="pq-card p-4 sm:p-6" data-testid="report-feedback">
          ...
          {r.feedback.recent.length === 0 ? (
            <p className="text-sm text-[#B9ABA2]">No feedback yet.</p>
          ) : (
